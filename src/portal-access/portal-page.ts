...
function openHashSection(){if(location.hash==='#repairs'){lastScreen='repairs';showScreen('repairs');if(!lists.repairs.initialized)loadCases('repairs',true);return true}if(location.hash==='#inspections'){lastScreen='inspections';showScreen('inspections');if(!lists.inspections.initialized)loadCases('inspections',true);return true}return false}window.addEventListener('hashchange',openHashSection);if(!openHashSection()){if(portalModel.focusedCase)openCase(portalModel.focusedCase.sourceRecordId);else showScreen('summary')}
`;
