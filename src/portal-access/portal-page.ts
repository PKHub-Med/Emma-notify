...
<div class="app">
  ...
  <main class="content"><div class="workspace">
    <div class="portal-refresh-bar" aria-live="polite">
      <button class="portal-refresh-button" id="portalRefreshButton" type="button"><span class="portal-refresh-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M20 11a8 8 0 1 0-2.34 5.66"/><path d="M20 4v7h-7"/></svg></span><span>Aktualizuj dane</span></button>
...
const REPAIR_STATUS_ICON_PATHS = [
  "M14.7 6.3a4 4 0 0 0-5-5l2.1 2.1-2.8 2.8L6.9 4.1a4 4 0 0 0 5 5l6.7 6.7a2 2 0 1 1-2.8 2.8L9.1 11.9",
  "m5 19 4-4",
] as const;
function repairIcon() { return `<svg viewBox="0 0 24 24">${REPAIR_STATUS_ICON_PATHS.map((path) => `<path d="${path}"/>`).join("")}</svg>`; }
...
function renderCaseMedia(item,detail){const media=node('div','case-detail-media-grid');const documents=node('section','case-detail-section media-section');documents.append(node('h3','','Dokument przeglądu'));if(item.documents?.length){const list=node('div','document-list');for(const asset of item.documents)list.append(documentLink(asset));documents.append(list,node('p','case-detail-hint','Kliknij dokument, aby otworzyć'))}else append(documents,node('strong','empty-media-title','Brak dokumentu'),node('p','empty-media-note','Dokument będzie dostępny po wykonaniu przeglądu.'));const photos=node('section','case-detail-section media-section');photos.append(node('h3','',item.photoLabel||'Zdjęcia z przeglądu'));if(item.photos?.length){const gallery=node('div','photo-gallery');for(const asset of item.photos){const button=node('button','photo-thumb');button.type='button';button.setAttribute('aria-label','Otwórz '+asset.title);const image=node('img','photo-thumb-image');image.src=fileUrl(asset.id,'thumb');image.alt=asset.title;image.loading='lazy';append(button,image,node('span','photo-caption',asset.title));button.addEventListener('click',()=>openPhoto(asset));gallery.append(button)}photos.append(gallery,node('p','case-detail-hint','Zobacz wszystkie zdjęcia →'))}else append(photos,node('strong','empty-media-title','Brak zdjęć'),node('p','empty-media-note','Zdjęcia nie zostały dodane do tego przeglądu.'));append(media,documents,photos);detail.append(media)}
...
gear:{style:'fill',paths:['M19.43 12.98c.04-.32.07-.65.07-.98s-.03-.66-.07-.98l2.11-1.65a.5.5 0 0 0 .12-.64l-2-3.46a.5.5 0 0 0-.61-.22l-2.49 1a7.4 7.4 0 0 0-1.69-.98l-.38-2.65A.5.5 0 0 0 14 2h-4a.5.5 0 0 0-.5.42l-.38 2.65a7.5 7.5 0 0 0-1.69.98l-2.49-1a.5.5 0 0 0-.61.22l-2 3.46a.5.5 0 0 0 .12.64l2.11 1.65a7.4 7.4 0 0 0-.08.98c0 .33.03.66.08.98l-2.11 1.65a.5.5 0 0 0-.12.64l2 3.46c.12.22.38.3.61.22l2.49-1c.52.4 1.08.73 1.69.98l.38 2.65c.04.24.25.42.5.42h4c.25 0 .46-.18.5-.42l.38-2.65c.61-.25 1.17-.58 1.69-.98l2.49 1c.23.08.49 0 .61-.22l2-3.46a.5.5 0 0 0-.12-.64l-2.11-1.65ZM12 15.5A3.5 3.5 0 1 1 12 8a3.5 3.5 0 0 1 0 7.5Z']},
person:['M12 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8Z','M4 21a8 8 0 0 1 16 0'],
repairStatus:${safeJson(REPAIR_STATUS_ICON_PATHS)},
repairExclamation:['M12 7v6','M12 17h.01'],
repairCalendar:{style:'stroke',nodes:[['rect',{x:'3',y:'5',width:'18',height:'16',rx:'2'}],['path',{d:'M16 3v4M8 3v4M3 10h18'}],['path',{d:'M8 14h.01M12 14h.01M16 14h.01M8 18h.01M12 18h.01M16 18h.01'}]]},
repairDocument:['M6 2h8l4 4v16H6z','M14 2v5h5','M9 12h6M9 16h6'],
repairCamera:{style:'stroke',nodes:[['path',{d:'M4 7h4l2-2h4l2 2h4v12H4z'}],['circle',{cx:'12',cy:'13',r:'3.5'}]]},
refresh:['M20 11a8 8 0 1 0-2.34 5.66','M20 4v7h-7']
};
const CASE_DETAIL_FILLED_ICONS={
location:{style:'fill',nodes:[['path',{d:'M12 22s7-6.2 7-13a7 7 0 1 0-14 0c0 6.8 7 13 7 13Z'}],['circle',{cx:'12',cy:'9',r:'2.2',fill:'#fff',stroke:'#fff'}]]},
document:{style:'fill',nodes:[['path',{d:'M6 2h8l4 4v16H6z'}],['path',{d:'M14 2v5h5',fill:'#fff',stroke:'#fff'}],['path',{d:'M9 13h6M9 17h6',stroke:'#fff'}]]},
camera:{style:'fill',nodes:[['path',{d:'M4 7h4l2-2h4l2 2h4v12H4z'}],['circle',{cx:'12',cy:'13',r:'3.5',fill:'#fff',stroke:'#fff'}]]}
};
function caseDetailSvg(name,style=null){const base=CASE_DETAIL_ICON_PATHS[name]||CASE_DETAIL_ICON_PATHS.info;const config=style==='fill'&&CASE_DETAIL_FILLED_ICONS[name]?CASE_DETAIL_FILLED_ICONS[name]:(Array.isArray(base)?{style:'stroke',paths:base}:base);const svg=document.createElementNS('http://www.w3.org/2000/svg','svg');svg.setAttribute('viewBox','0 0 24 24');svg.dataset.iconStyle=config.style||'stroke';const nodes=config.nodes||config.paths.map(d=>['path',{d}]);for(const [tag,attributes]of nodes){const element=document.createElementNS('http://www.w3.org/2000/svg',tag);for(const [key,value]of Object.entries(attributes))element.setAttribute(key,value);svg.append(element)}return svg}
function caseDetailIcon(name,className='',style=null){const base=CASE_DETAIL_ICON_PATHS[name]||CASE_DETAIL_ICON_PATHS.info;const iconStyle=style||(Array.isArray(base)?'stroke':base.style);const wrap=node('span','case-detail-icon is-'+iconStyle+' '+className);const svg=caseDetailSvg(name,style);svg.setAttribute('aria-hidden','true');wrap.append(svg);return wrap}
function caseDetailVariant(d){return CASE_DETAIL_VARIANTS[d.variant]||CASE_DETAIL_VARIANTS.VERIFY}
function setCaseDetailShell(active){const workspace=document.querySelector('.workspace');workspace?.classList.toggle('case-detail-active',active);workspace?.classList.remove('case-detail-repair-active');document.getElementById('caseCard')?.classList.toggle('case-detail-screen',active);document.getElementById('caseBack').hidden=active;document.getElementById('caseScreenDeviceLink').hidden=active;const refreshBar=refreshButton.closest('.portal-refresh-bar');if(!active&&workspace&&refreshBar)workspace.prepend(refreshBar)}
function caseDetailStatusPill(label,d,className=''){const config=caseDetailVariant(d);const pill=node('span','case-detail-status-pill is-'+config.tone+' '+className);append(pill,caseDetailIcon(config.icon),node('span','',text(label)));return pill}
function caseDetailHeader(listLabel,cardLabel,title,statusPill,subtitle,metaText,repair=false){const head=node('header','case-detail-header case-detail-head'+(repair?' case-detail-repair-head':''));const breadcrumb=node('div','case-detail-breadcrumb');const back=node('button','',listLabel);back.type='button';back.addEventListener('click',()=>history.back());append(breadcrumb,back,node('span','','›'),node('span','',cardLabel));const titleRow=node('div','case-detail-title-row');append(titleRow,node('h1','',title),statusPill);const main=node('div','case-detail-header-main case-detail-head-copy');append(main,breadcrumb,titleRow,node('p','case-detail-subtitle',subtitle));const actions=node('div','case-detail-header-actions');const meta=node('div','case-detail-head-meta');if(metaText)meta.append(node('p','',metaText));actions.append(meta);const refreshBar=refreshButton.closest('.portal-refresh-bar');if(refreshBar)actions.append(refreshBar);append(head,main,actions);return head}
function caseDetailSectionHeader(title,iconName,action=null,iconStyle=null){const header=node('div','case-detail-section-head');const titleWrap=node('div','case-detail-section-title');append(titleWrap,caseDetailIcon(iconName,'',iconStyle),node('h3','',title));header.append(titleWrap);if(action)header.append(action);return header}
function caseDetailSummaryTile(label,value,iconName,help=''){const tile=node('div','case-detail-summary');append(tile,caseDetailIcon(iconName));const copy=node('div','case-detail-summary-copy');append(copy,node('span','',label),node('strong','',text(value)));if(help)copy.append(node('small','',help));tile.append(copy);return tile}
function caseDetailDate(value){if(!value)return null;const parsed=new Date(value);return Number.isNaN(parsed.getTime())?value:formatDate(parsed)}
function caseDetailDeviceTile(label,value,content=null){const tile=node('div','case-detail-pair');tile.append(node('span','',label));if(content)tile.append(content);else tile.append(node('strong','',text(value)));return tile}
function caseDetailRelatedCard(label,iconName,action){const button=featureButton('',action,'case-detail-link');append(button,caseDetailIcon(iconName),node('span','',label),node('span','case-detail-link-arrow','→'));return button}
function caseDetailPanelCopy(value){const copy=node('p','case-detail-panel-description',text(value,''));return copy}
function caseDetailPanelSide(d){if(d.relatedRepairNumber){const card=node('div','case-detail-panel-side case-detail-repair-card');append(card,caseDetailIcon('wrench'),node('strong','','Powiązana naprawa'),featureButton('Naprawa #'+d.relatedRepairNumber+' →',null,'case-detail-related-action'));return card}if(d.variant==='SCHEDULED'){const card=node('div','case-detail-panel-side');append(card,caseDetailIcon('calendar'),node('span','','Planowany termin przeglądu'),node('strong','',formatDate(d.scheduledAt)));return card}if(d.variant==='DUE'){const card=node('div','case-detail-panel-side');append(card,caseDetailIcon('calendar'),node('span','','Zaplanowanie przeglądu'),featureButton('Umów termin',null,'case-detail-panel-action'));return card}if(d.variant==='PROBLEM'&&d.requiredAction){const card=node('div','case-detail-panel-side');append(card,caseDetailIcon('info'),node('strong','','Wymagane działanie'),caseDetailPanelCopy(d.requiredAction));return card}return null}
function caseDetailStatusPanel(d){const config=caseDetailVariant(d);const titles={PASSED:'Przegląd aktualny',CONDITIONAL:'Uwagi z przeglądu',FAILED:'Urządzenie niesprawne',SCHEDULED:'Przegląd umówiony',DUE:'Przegląd do realizacji',PROBLEM:'Przegląd nie został wykonany',VERIFY:'Dane wymagają weryfikacji'};const descriptions={PASSED:d.heroDescription,CONDITIONAL:d.notes||d.faults||d.heroDescription,FAILED:d.faults||d.heroDescription,SCHEDULED:d.heroDescription,DUE:d.heroDescription,PROBLEM:d.failureReason||d.heroDescription,VERIFY:'Szczegóły przeglądu są chwilowo niedostępne.'};const panel=node('div','case-detail-status-panel is-'+config.tone);const main=node('div','case-detail-panel-main');main.append(caseDetailIcon(config.icon,'case-detail-panel-icon'));const copy=node('div');append(copy,node('strong','case-detail-panel-title',titles[d.variant]||titles.VERIFY),caseDetailPanelCopy(descriptions[d.variant]||''));if(d.variant==='CONDITIONAL'&&d.faults&&d.faults!==descriptions.CONDITIONAL)copy.append(caseDetailPanelCopy(d.faults));append(main,copy);panel.append(main);const side=caseDetailPanelSide(d);if(side)panel.append(side);return panel}
function caseDetailResultTarget(d){const section=node('section','case-detail-section case-detail-result-section');section.append(caseDetailSectionHeader('Wynik przeglądu','check'));const performed=d.performedAt?formatDate(d.performedAt):d.scheduledAt?formatDate(d.scheduledAt):null;const dateHelp=!performed&&d.variant==='DUE'?'Nie wyznaczono terminu.':'';const resultHelp=!d.result&&(d.variant==='DUE'||d.variant==='SCHEDULED')?'Wynik będzie dostępny po wykonaniu przeglądu.':'';const validHelp=!d.validUntil&&!d.validUntilLabel&&(d.variant==='DUE'||d.variant==='SCHEDULED')?'Data zostanie ustalona po wykonaniu przeglądu.':'';const grid=node('div','case-detail-result-grid');append(grid,caseDetailSummaryTile('Data '+(d.performedAt?'wykonania':'planowana'),performed,'calendar',dateHelp),caseDetailSummaryTile('Wynik',d.result,'shield',resultHelp),caseDetailSummaryTile('Ważny do',d.validUntilLabel||(d.validUntil?formatDate(d.validUntil):null),'calendar',validHelp));append(section,grid,caseDetailStatusPanel(d));return section}
function caseDetailDeviceSection(d){const section=node('section','case-detail-section case-detail-device-data');const deviceAction=d.device.id?()=>{setCaseDetailShell(false);openDevice(d.device.id)}:null;const fullCard=deviceAction?featureButton('Zobacz pełną kartę urządzenia →',deviceAction,'case-detail-section-action'):null;section.append(caseDetailSectionHeader('Dane urządzenia','device',fullCard));const grid=node('div','case-detail-grid');append(grid,caseDetailDeviceTile('Typ sprzętu',d.device.name),caseDetailDeviceTile('Producent',d.device.manufacturer),caseDetailDeviceTile('Model',d.device.model),caseDetailDeviceTile('Numer seryjny',d.device.serialNumber),caseDetailDeviceTile('Numer inwentarzowy',d.device.inventoryNumber));const rfid=node('div','case-detail-rfid-value');if(d.device.epc){append(rfid,node('span','case-detail-rfid-dot'),node('strong','','Urządzenie oznakowane'));const epc=node('button','case-detail-epc','Pokaż kod EPC →');epc.type='button';epc.addEventListener('click',()=>{epc.textContent=epc.textContent.startsWith('Pokaż')?d.device.epc:'Pokaż kod EPC →'});rfid.append(epc)}else rfid.append(node('strong','',text(d.device.tagged)));grid.append(caseDetailDeviceTile('RFID / EPC',null,rfid),caseDetailDeviceTile('Rok produkcji',d.device.productionYear),caseDetailDeviceTile('Data uruchomienia',caseDetailDate(d.device.commissionedAt)),caseDetailDeviceTile('Gwarancja',caseDetailDate(d.device.warrantyUntil)));section.append(grid);return section}
function caseDetailLocationSection(d,iconStyle=null){const section=node('section','case-detail-section case-detail-location');section.append(caseDetailSectionHeader('Lokalizacja','location',null,iconStyle));const grid=node('div','case-detail-grid');append(grid,caseDetailDeviceTile('Szpital',d.location.hospital),caseDetailDeviceTile('Oddział',d.location.department));section.append(grid);return section}
function caseDetailMediaEmpty(iconName,title,description){const empty=node('div','case-detail-media-empty');append(empty,caseDetailIcon(iconName),node('div',''),node('strong','',title),node('p','',description));return empty}
function renderInspectionMediaTarget(item,detail){const media=node('div','case-detail-media-grid');const documents=node('section','case-detail-section media-section');documents.append(caseDetailSectionHeader('Dokument przeglądu','document'));if(item.documents?.length){const list=node('div','document-list');for(const asset of item.documents)list.append(documentLink(asset));documents.append(list)}else documents.append(caseDetailMediaEmpty('document','Brak dokumentu','Dokument będzie dostępny po wykonaniu przeglądu.'));const photos=node('section','case-detail-section media-section');const count=item.photos?.length||0;photos.append(caseDetailSectionHeader(item.photoLabel||'Zdjęcia z przeglądu','camera',count?node('span','case-detail-photo-count',count+' '+(count===1?'zdjęcie':count<5?'zdjęcia':'zdjęć')):null));if(count){const gallery=node('div','photo-gallery');if(count>4)gallery.classList.add('is-collapsed');for(const asset of item.photos){const button=node('button','photo-thumb');button.type='button';button.setAttribute('aria-label','Otwórz '+asset.title);const image=node('img','photo-thumb-image');image.src=fileUrl(asset.id,'thumb');image.alt=asset.title;image.loading='lazy';append(button,image);button.addEventListener('click',()=>openPhoto(asset));gallery.append(button)}photos.append(gallery);if(count>4){const more=node('button','case-detail-gallery-more','Zobacz wszystkie zdjęcia →');more.type='button';more.addEventListener('click',()=>{const expanded=gallery.classList.toggle('is-expanded');more.textContent=expanded?'Pokaż mniej':'Zobacz wszystkie zdjęcia →'});photos.append(more)}}else photos.append(caseDetailMediaEmpty('camera','Brak zdjęć','Zdjęcia nie zostały dodane do tego przeglądu.'));append(media,documents,photos);detail.append(media)}
function caseDetailRelatedSection(d){const section=node('section','case-detail-section case-detail-links');section.append(caseDetailSectionHeader('Powiązane informacje','link'));const grid=node('div','case-detail-links-grid');const action=d.device.id?()=>{setCaseDetailShell(false);openDevice(d.device.id)}:null;append(grid,caseDetailRelatedCard('Zobacz kartę urządzenia','device',action),caseDetailRelatedCard('Zobacz historię przeglądów','clock',action),caseDetailRelatedCard('Zobacz historię napraw','wrench',action));section.append(grid);return section}
function renderInspectionDetail(item,detail){const d=item.inspectionDetails;setCaseDetailShell(true);document.getElementById('casePageTitle').textContent='Przegląd #'+text(d.number);const subtitle=d.variant==='PROBLEM'?'Przegląd nie został wykonany.':'Wynik przeglądu oraz szczegóły dla wybranego urządzenia.';const metaText=d.headerDateType&&d.headerDate?d.headerDateType+': '+formatDateTime(d.headerDate):'';const head=caseDetailHeader('Przeglądy','Karta przeglądu','Przegląd #'+text(d.number),caseDetailStatusPill(d.status,d),subtitle,metaText);detail.append(head);if(!d.verified){detail.append(caseDetailStatusPanel(d));renderInspectionMediaTarget(item,detail);return}const hero=node('section','case-detail-hero is-'+caseDetailVariant(d).tone);const deviceIcon=node('div','case-detail-device-icon');deviceIcon.append(caseDetailIcon('device'));const device=node('div','case-detail-device');append(device,node('h2','',d.device.name),node('p','case-detail-device-sub',text(d.device.manufacturer)+' • '+text(d.device.model)),node('p','case-detail-device-id','SN: '+text(d.device.serialNumber)+'  |  Nr inw.: '+text(d.device.inventoryNumber)));const result=node('div','case-detail-result');append(result,caseDetailStatusPill(d.heroLabel,d,'is-large'),node('p','',text(d.heroDescription,'')));append(hero,deviceIcon,device,result);detail.append(hero,caseDetailResultTarget(d),caseDetailDeviceSection(d),caseDetailLocationSection(d));renderInspectionMediaTarget(item,detail);detail.append(caseDetailRelatedSection(d))}
function setCaseDetailNavigation(screen){navButtons.forEach(button=>button.classList.toggle('active',button.dataset.screen===screen))}
function repairStatusPill(label,className=''){const pill=node('span','case-detail-status-pill is-info '+className);append(pill,caseDetailIcon('repairStatus'),node('span','',text(label)));return pill}
function repairDeviceStatusVariant(status){const normalized=text(status,'').toLocaleUpperCase('pl-PL');if(normalized==='SPRAWNY')return{tone:'success',icon:'check'};if(normalized==='WARUNKOWO DOPUSZCZONY')return{tone:'warning',icon:'repairExclamation'};if(normalized==='NIESPRAWNY')return{tone:'danger',icon:'repairExclamation'};if(normalized==='WYCOFANY Z UŻYTKU'||normalized==='SKASOWANY')return{tone:'neutral',icon:'repairExclamation'};return{tone:'neutral',icon:null}}
function renderRepairMedia(item,detail){const media=node('div','case-detail-media-grid case-detail-repair-media');const documents=node('section','case-detail-section media-section');documents.append(caseDetailSectionHeader('Dokumenty naprawy','document',null,'fill'));if(item.documents?.length){const list=node('div','document-list');for(const asset of item.documents)list.append(documentLink(asset));documents.append(list)}else documents.append(caseDetailMediaEmpty('repairDocument','Brak dokumentów.','Dokumenty będą dostępne po dodaniu.'));const photos=node('section','case-detail-section media-section');const count=item.photos?.length||0;photos.append(caseDetailSectionHeader('Zdjęcia z naprawy','camera',count?node('span','case-detail-photo-count',String(count)):null,'fill'));if(count){const gallery=node('div','photo-gallery');for(const asset of item.photos){const button=node('button','photo-thumb');button.type='button';button.setAttribute('aria-label','Otwórz '+asset.title);const image=node('img','photo-thumb-image');image.src=fileUrl(asset.id,'thumb');image.alt=asset.title;image.loading='lazy';append(button,image);button.addEventListener('click',()=>openPhoto(asset));gallery.append(button)}photos.append(gallery)}else photos.append(caseDetailMediaEmpty('repairCamera','Brak zdjęć.','Zdjęcia będą dostępne po dodaniu.'));append(media,documents,photos);detail.append(media)}
function renderRepairDetail(item,detail){const d=item.repairDetails;setCaseDetailShell(true);document.querySelector('.workspace')?.classList.add('case-detail-repair-active');setCaseDetailNavigation('repairs');document.getElementById('casePageTitle').textContent='Naprawa #'+text(d.number);const metaText=d.reportedAt?'Zgłoszono: '+(d.reportedAtDateOnly?formatDate(d.reportedAt):formatDateTime(d.reportedAt)):'';const head=caseDetailHeader('Naprawy','Karta naprawy','Naprawa #'+text(d.number),repairStatusPill(d.heroLabel),text(d.heroDescription,''),metaText,true);const hero=node('section','case-detail-hero case-detail-repair-hero');const deviceIcon=node('div','case-detail-device-icon');deviceIcon.append(caseDetailIcon('device'));const device=node('div','case-detail-device');append(device,node('h2','',d.device.name),node('p','case-detail-device-sub',text(d.device.manufacturer)+' • '+text(d.device.model)),node('p','case-detail-device-id','SN: '+text(d.device.serialNumber)+'  |  Nr inw.: '+text(d.device.inventoryNumber)));const state=node('div','case-detail-repair-state');append(state,node('span','','Stan urządzenia'));const statusVariant=repairDeviceStatusVariant(d.device.status);const statePill=node('div','case-detail-repair-state-pill is-'+statusVariant.tone);if(statusVariant.icon)statePill.append(caseDetailIcon(statusVariant.icon));statePill.append(node('strong','',text(d.device.status)));state.append(statePill);append(hero,deviceIcon,device,state);const details=node('section','case-detail-section case-detail-repair-summary');details.append(caseDetailSectionHeader('Szczegóły naprawy','gear'));const grid=node('div','case-detail-result-grid');append(grid,caseDetailSummaryTile('Data zgłoszenia',d.reportedAt?formatDate(d.reportedAt):null,'repairCalendar'),caseDetailSummaryTile('Numer zgłoszenia',d.number?'#'+d.number:null,'repairDocument'),caseDetailSummaryTile('Nr zlecenia / WZ',d.clientOrderNumber,'repairCalendar'),caseDetailSummaryTile('Zgłaszający',d.reporter,'person'),caseDetailSummaryTile('Nr oferty',d.offerNumber,'repairDocument'),caseDetailSummaryTile('Data zakończenia',d.completedAt?formatDate(d.completedAt):null,'repairCalendar',d.completedAt?'':'Data zostanie uzupełniona po zakończeniu naprawy.'));details.append(grid);const notes=node('section','case-detail-section case-detail-repair-notes');notes.append(caseDetailSectionHeader('Uwagi / diagnostyka / opis usterki','repairDocument'));const descriptions=[d.faultDescription,d.repairDescription].filter(value=>typeof value==='string'&&value.trim());notes.append(node('div','case-detail-repair-notes-box',descriptions.length?descriptions.join('\n\n'):'Brak opisu.\nInformacje z diagnostyki będą dostępne po wykonaniu diagnozy.'));detail.append(head,hero,details,caseDetailDeviceSection(d),caseDetailLocationSection(d,'fill'),notes);renderRepairMedia(item,detail);const related=node('section','case-detail-section case-detail-links');related.append(caseDetailSectionHeader('Powiązane informacje','link'));const relatedGrid=node('div','case-detail-links-grid');const deviceAction=d.device.id?()=>{setCaseDetailShell(false);openDevice(d.device.id)}:()=>showListScreen('devices');const repairHistory=d.device.id?deviceAction:()=>showListScreen('repairs');const inspectionHistory=d.device.id?deviceAction:()=>showListScreen('inspections');append(relatedGrid,caseDetailRelatedCard('Zobacz kartę urządzenia','device',deviceAction),caseDetailRelatedCard('Zobacz historię napraw','clock',repairHistory),caseDetailRelatedCard('Zobacz powiązane przeglądy','wrench',inspectionHistory));related.append(relatedGrid);detail.append(related)}
function renderCase(item){const inspection=item.type==='INSPECTION'&&item.inspectionDetails;const repair=item.type==='REPAIR'&&item.repairDetails;setCaseDetailShell(Boolean(inspection||repair));const deviceButton=document.getElementById('caseScreenDeviceLink');deviceButton.hidden=Boolean(inspection||repair)||!item.deviceId;const detail=document.getElementById('caseDetail');detail.replaceChildren();if(inspection){setCaseDetailNavigation('inspections');renderInspectionDetail(item,detail);return}if(repair){renderRepairDetail(item,detail);return}document.getElementById('casePageTitle').textContent='Numer Sprawy: '+text(item.caseNumber);const header=node('div','case-header-line');append(header,node('span','case-kind','Naprawa'),node('span','case-service-inline','Serwis: '+portalModel.serviceProviderName));const hero=node('div','case-hero');const titleLine=node('div','hero-title-line');const inline=node('div','inline-status case-main-status');append(inline,node('label','','AKTUALNY STATUS'),status(item.currentStatus));append(titleLine,node('h2','',item.deviceName),inline);const grid=node('div','meta-grid case-meta-grid');append(grid,meta('Numer Sprawy',item.caseNumber),meta('Numer zlecenia klienta',text(item.clientOrderNumber,'brak numeru')),meta('Numer seryjny',item.serialNumber),meta('Nr inwentarzowy',item.inventoryNumber),meta('Oddział',item.department),meta('Data zgłoszenia',item.reportedAtDateOnly?formatDate(item.reportedAt):formatDateTime(item.reportedAt)));const description=meta('Usterka / opis',item.description);description.style.marginTop='9px';append(hero,titleLine,grid,description);append(detail,header,hero);renderCaseMedia(item,detail);renderHistory(item,detail)}
