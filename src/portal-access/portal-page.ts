...
function renderCase(item){document.getElementById('casePageTitle').textContent='Numer Sprawy: '+text(item.caseNumber);const deviceButton=document.getElementById('caseScreenDeviceLink');deviceButton.hidden=!item.deviceId;deviceButton.textContent=item.deviceId?item.deviceName+' → karta urządzenia':'Przejdź do karty urządzenia';const detail=document.getElementById('caseDetail');detail.replaceChildren();if(item.type==='INSPECTION'&&item.inspectionDetails){renderInspectionV5(item,detail);return}const header=node('div','case-header-line');append(header,node('span','case-kind','Naprawa'),node('span','case-service-inline','Serwis: '+portalModel.serviceProviderName));const hero=node('div','case-hero');const titleLine=node('div','hero-title-line');const inline=node('div','inline-status case-main-status');append(inline,node('label','','AKTUALNY STATUS'),status(item.currentStatus));append(titleLine,node('h2','',item.deviceName),inline);const grid=node('div','meta-grid case-meta-grid');append(grid,meta('Numer Sprawy',item.caseNumber),meta('Numer zlecenia klienta',text(item.clientOrderNumber,'brak numeru')),meta('Numer seryjny',item.serialNumber),meta('Nr inwentarzowy',item.inventoryNumber),meta('Oddział',item.department),meta('Data zgłoszenia',item.reportedAtDateOnly?formatDate(item.reportedAt):formatDateTime(item.reportedAt)));const description=meta('Usterka / opis',item.description);description.style.marginTop='9px';append(hero,titleLine,grid,description);append(detail,header,hero);renderCaseMedia(item,detail);renderHistory(item,detail)}
function renderInspectionV5(item,detail){const d=item.inspectionDetails;document.getElementById('casePageTitle').textContent='Przegląd #'+text(d.number);const head=node('div','inspection-v5-head');const headCopy=node('div');append(headCopy,node('div','inspection-v5-kicker','Przeglądy › Karta przeglądu'),node('h2','','Przegląd #'+text(d.number)),node('p','inspection-v5-subtitle',d.variant==='PROBLEM'?'Szczegóły niewykonanego przeglądu.':'Wynik przeglądu oraz szczegóły dla wybranego urządzenia.'));const headMeta=node('div','inspection-v5-head-meta');if(d.headerDateType&&d.headerDate)headMeta.append(node('p','',d.headerDateType+': '+formatDateTime(d.headerDate)));headMeta.append(status(d.status));append(head,headCopy,headMeta);detail.append(head);if(!d.verified){const safe=node('section','inspection-v5-safe');append(safe,node('h3','','Dane wymagają weryfikacji'),node('p','','Szczegóły przeglądu są chwilowo niedostępne. Skontaktuj się z serwisem lub odśwież dane później.'));detail.append(safe);renderCaseMedia(item,detail);return}const hero=node('section','inspection-v5-hero '+inspectionTone(d));const icon=node('div','inspection-v5-device-icon','▣');icon.setAttribute('aria-hidden','true');const left=node('div','inspection-v5-device');append(left,node('h3','',d.device.name),node('p','inspection-v5-device-sub',text(d.device.manufacturer)+' • '+text(d.device.model)),node('p','inspection-v5-device-id','SN: '+text(d.device.serialNumber)+' | Nr inw.: '+text(d.device.inventoryNumber)));const right=node('div','inspection-v5-result');append(right,node('span','inspection-v5-result-label',text(d.heroLabel)),node('p','',text(d.heroDescription,'')));append(hero,icon,left,right);detail.append(hero);if(d.status==='DO REALIZACJI'){const callout=node('section','inspection-v5-callout');append(callout,node('h3','','Umów termin'),node('p','','Skontaktuj się z serwisem, aby ustalić termin przeglądu.'),featureButton('Umów termin',null,'inspection-v5-action'));detail.append(callout)}if(d.status==='PROBLEM'){detail.append(v5Section('Przegląd niewykonany',[['Powód niewykonania',d.failureReason],['Uwagi',d.notes],['Wymagane działanie',d.requiredAction]]))}else{detail.append(v5ResultSection(d));if(d.notes||d.faults)detail.append(v5Section('Uwagi i usterki',[['Uwagi',d.notes],['Usterki',d.faults]]))}if(d.relatedRepairNumber){const repair=node('section','inspection-v5-related');append(repair,node('span','','Powiązana naprawa'),featureButton('Naprawa #'+d.relatedRepairNumber+' →',null,'inspection-v5-related-action'));detail.append(repair)}detail.append(v5Section('Dane urządzenia',[['Typ sprzętu',d.device.name],['Producent',d.device.manufacturer],['Model',d.device.model],['Numer seryjny',d.device.serialNumber],['Numer inwentarzowy',d.device.inventoryNumber],['RFID / EPC',d.device.tagged],['Rok produkcji',d.device.productionYear],['Data uruchomienia',d.device.commissionedAt],['Gwarancja',d.device.warrantyUntil]]));if(d.device.epc){const rfid=node('section','inspection-v5-rfid');append(rfid,node('h3','','Urządzenie oznakowane'));const button=node('button','inspection-v5-epc','Pokaż kod EPC →');button.type='button';button.addEventListener('click',()=>{button.textContent=button.textContent.startsWith('Pokaż')?d.device.epc:'Pokaż kod EPC →'});rfid.append(button);detail.append(rfid)}detail.append(v5Section('Lokalizacja',[['Szpital',d.location.hospital],['Oddział',d.location.department]]));renderCaseMedia(item,detail);const links=node('section','inspection-v5-links');links.append(node('h3','','Powiązane informacje'));const linkGrid=node('div','inspection-v5-links-grid');const deviceAction=d.device.id?()=>openDevice(d.device.id):null;for(const label of ['Zobacz kartę urządzenia →','Zobacz historię przeglądów →','Zobacz historię napraw →'])linkGrid.append(featureButton(label,deviceAction,'inspection-v5-link'));links.append(linkGrid);detail.append(links)}
function v5ResultSection(d){const section=node('section','inspection-v5-section inspection-v5-result-section');section.append(node('h3','','Wynik przeglądu'));const grid=node('div','inspection-v5-result-grid');const performed=d.performedAt?formatDate(d.performedAt):d.scheduledAt?formatDate(d.scheduledAt):null;grid.append(v5Summary('Data '+(d.performedAt?'wykonania':'planowana'),performed),v5Summary('Wynik',d.result),v5Summary('Ważny do',d.validUntilLabel||(d.validUntil?formatDate(d.validUntil):null)));section.append(grid);if(d.admission){const admission=node('div','inspection-v5-admission');append(admission,node('strong','','Dopuszczenie do użytku'),node('p','',d.admission));section.append(admission)}return section}
function v5Summary(label,value){const box=node('div','inspection-v5-summary');append(box,node('span','',label),node('strong','',text(value)));return box}
...
function inspectionTone(d){if(d.variant==='FAILED'||d.variant==='PROBLEM')return'tone-red';if(d.variant==='CONDITIONAL')return'tone-amber';if(d.variant==='PASSED')return'tone-green';if(d.variant==='SCHEDULED')return'tone-blue';return'tone-slate'}
function v5Pair(label,value){const row=node('div','inspection-v5-pair');append(row,node('span','',label),node('strong','',text(value)));return row}
function v5Section(title,rows){const section=node('section','inspection-v5-section');section.dataset.section=title;if(title==='Dane urządzenia')section.classList.add('inspection-v5-device-data');if(title==='Lokalizacja')section.classList.add('inspection-v5-location');section.append(node('h3','',title));const grid=node('div','inspection-v5-grid');for(const [label,value]of rows)grid.append(v5Pair(label,value));section.append(grid);return section}
