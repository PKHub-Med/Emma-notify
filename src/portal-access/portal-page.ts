...
function renderInspectionDetail(item,detail){const d=item.inspectionDetails;setCaseDetailShell(true);document.getElementById('casePageTitle').textContent='Przegląd #'+text(d.number);const head=node('header','case-detail-head');const breadcrumb=node('div','case-detail-breadcrumb');const back=node('button','','Przeglądy');back.type='button';back.addEventListener('click',()=>history.back());append(breadcrumb,back,node('span','','›'),node('span','','Karta przeglądu'));const titleRow=node('div','case-detail-title-row');append(titleRow,node('h1','','Przegląd #'+text(d.number)),caseDetailStatusPill(d.status,d));const subtitle=d.variant==='PROBLEM'?'Przegląd nie został wykonany.':'Wynik przeglądu oraz szczegóły dla wybranego urządzenia.';const copy=node('div','case-detail-head-copy');append(copy,breadcrumb,titleRow,node('p','case-detail-subtitle',subtitle));const meta=node('div','case-detail-head-meta');if(d.headerDateType&&d.headerDate)meta.append(node('p','',d.headerDateType+': '+formatDateTime(d.headerDate)));append(head,copy,meta);detail.append(head);if(!d.verified){detail.append(caseDetailStatusPanel(d));renderInspectionMediaTarget(item,detail);return}const hero=node('section','case-detail-hero is-'+caseDetailVariant(d).tone);const deviceIcon=node('div','case-detail-device-icon');deviceIcon.append(caseDetailIcon('device'));const device=node('div','case-detail-device');append(device,node('h2','',d.device.name),node('p','case-detail-device-sub',text(d.device.manufacturer)+' • '+text(d.device.model)),node('p','case-detail-device-id','SN: '+text(d.device.serialNumber)+'  |  Nr inw.: '+text(d.device.inventoryNumber)));const result=node('div','case-detail-result');append(result,caseDetailStatusPill(d.heroLabel,d,'is-large'),node('p','',text(d.heroDescription,'')));append(hero,deviceIcon,device,result);detail.append(hero,caseDetailResultTarget(d),caseDetailDeviceSection(d),caseDetailLocationSection(d));renderInspectionMediaTarget(item,detail);detail.append(caseDetailRelatedSection(d))}
