...
try {
  ...
  for (let index = 0; index < missing.length; index++) {
    ...
    if (!result.ok) {
      stats.failedReads++;
      console.error(`READ_FAIL ${row.airtableRecordId}: ${result.error}`);
      continue;
    }

    if (result.value === null) {
      stats.sourceMissing++;
      // Keep summary-only output for missing source values.
      continue;
    }
    ...
    if (mode === "dry-run") {
      if ((index + 1) % 100 === 0 || index === missing.length - 1) {
        console.log(
          `Progress: ${index + 1}/${missing.length} | wouldUpdate=${stats.wouldUpdate} | sourceMissing=${stats.sourceMissing} | failedReads=${stats.failedReads}`
        );
      }
      continue;
    }
    ...
    try {
      ...
      if ((index + 1) % 100 === 0 || index === missing.length - 1) {
        console.log(
          `Progress: ${index + 1}/${missing.length} | updated=${stats.updated} | sourceMissing=${stats.sourceMissing} | failedReads=${stats.failedReads} | failedWrites=${stats.failedWrites}`
        );
      }
    } catch (error) {
      stats.failedWrites++;
  ...
  try {
    ...
    if (!process.exitCode) process.exitCode = 11;
  }
}
'@
